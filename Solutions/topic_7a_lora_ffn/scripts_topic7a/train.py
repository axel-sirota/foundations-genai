"""
train.py -- LoRA fine-tuning of Flan-T5-small on Barclays complaint summarization.

Task:  Given a customer complaint, generate a 1-sentence summary.
Model: google/flan-t5-small (77M params, fits on a T4 16 GB GPU).
PEFT:  LoraConfig with TaskType.SEQ_2_SEQ_LM, target q and v projections.
Metrics: Inline ROUGE-1 token-overlap approximation in numpy.
         No evaluate library (incompatible with datasets 4.x).

SageMaker container: transformers 4.56.2, pytorch 2.8.0, py312.
Container requirements.txt installs peft and numpy<2.
...
COMPLAINTS = [
    ...
    ("I transferred 2000 pounds to my sister account two days ago and the money has not "
...
def token_overlap_f1(pred_ids, label_ids, tokenizer):
    ...
    for pred, label in zip(pred_ids, label_ids):
        ...
        pred = [t for t in pred if t not in (tokenizer.pad_token_id, tokenizer.eos_token_id)]
